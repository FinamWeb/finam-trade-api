"""Runnable SMA 9/30 example built on the published ``finam-sdk`` package."""
...
def _history(client: FinamClient, args: argparse.Namespace) -> tuple[list[Decimal], Bar]:
...
def _position(client: FinamClient, account_id: str, symbol: str) -> Decimal:
...
def run(client: FinamClient, args: argparse.Namespace) -> None:
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    try:
        with FinamClient(secret=args.secret) as client:
            run(client, args)
