...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| limit | integer | no | Лимит количества транзакций |
...
const response = await fetch(`https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions?${params}`, {
  method: 'GET',
  headers: {
...
response = requests.get(
    "https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions",
    headers={
...
func main() {
    ...
    req, _ := http.NewRequest("GET", "https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions?"+params.Encode(), nil)
...
$ch = curl_init('https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions?' . http_build_query(['limit' => 'YOUR_LIMIT', 'interval.start_time' => 'YOUR_INTERVAL.START_TIME', 'interval.end_time' => 'YOUR_INTERVAL.END_TIME']));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .get("https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions")
        .bearer_auth(token)
...
curl -X GET 'https://api.finam.ru/v1/accounts/YOUR_ACCOUNT_ID/transactions?limit=YOUR_LIMIT&interval.start_time=YOUR_INTERVAL.START_TIME&interval.end_time=YOUR_INTERVAL.END_TIME' \
