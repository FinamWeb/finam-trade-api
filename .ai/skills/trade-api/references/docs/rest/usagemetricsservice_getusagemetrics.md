...
const response = await fetch('https://api.finam.ru/v1/usage', {
  method: 'GET',
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("GET", "https://api.finam.ru/v1/usage", nil)
...
$ch = curl_init('https://api.finam.ru/v1/usage');
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .get("https://api.finam.ru/v1/usage")
        .bearer_auth(token)
        .send().await?;
...
curl -X GET 'https://api.finam.ru/v1/usage' \
